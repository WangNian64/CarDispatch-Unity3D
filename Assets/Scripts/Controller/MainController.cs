...
public class MainController : MonoBehaviour
{
    ...
    void Update() 
    {
        ...
        if (GlobalVaribles.allocableTasks.Count > 0 && GlobalVaribles.allocableCars.Count > 0)
        {
            ...
            else//Task<Car，会有空闲的小车，此时需要进行 多选少，再进行全排列
            {
                ...
                {
                    List<Dictionary<Car, Task>> matchList = new List<Dictionary<Car, Task>>();
                    getMatchList(ref matchList, carList, trueTasks, 0, carList.Length);
                    //计算matchList中的最小匹配minMatch
                    Dictionary<Car, Task> aMinMatch = new Dictionary<Car, Task>();
                    float aMinTime = getMinMatch(matchList, ref aMinMatch);
    ...
    public float calcuTasksTime(Dictionary<Car, Task> match)
    {
        ...
        SimuCarController[] allSCC = new SimuCarController[match.Count];
        int i = 0;
